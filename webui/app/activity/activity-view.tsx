  ...
  return (
    <Card className="frost frost-elevated flex h-[calc(100vh-9rem)] flex-col overflow-hidden">
      <CardHeader className="flex flex-row flex-wrap items-center gap-3 space-y-0 border-b py-3">
        {/*
          The badge sits beside the heading rather than inside it — nested, its
          text became part of the heading's accessible name ("ActivityLive").
        */}
        <div className="flex items-center gap-2">
          <CardTitle className="text-headline">Activity</CardTitle>
          <span
            className={cn(
      ...
      <CardContent className="min-h-0 flex-1 p-0">
        <pre
          ref={viewport}
          tabIndex={0}
          // Without a role the aria-label is ignored on a <pre>, leaving the
          // scroll region unnamed. `inset` keeps the focus ring inside the
          // card, whose overflow-hidden was clipping the offset ring entirely.
          role="log"
          ...
          className="h-full overflow-auto whitespace-pre-wrap break-words bg-muted/30 p-4 font-mono text-caption leading-relaxed text-muted-foreground focus-visible:ring-inset"
        >
          {visible.length === 0 ? (
            filter ? (
